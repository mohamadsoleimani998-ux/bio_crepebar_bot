...
CREATE TABLE IF NOT EXISTS orders (
  ...
  status          TEXT NOT NULL DEFAULT 'draft',   -- draft|submitted|paid|canceled
...
CREATE TABLE IF NOT EXISTS wallet_transactions (
  ...
  user_id     BIGINT NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
  kind        TEXT NOT NULL,   -- topup | order | refund | cashback | adjust | game
...
def set_phone(tg_id: int, phone: str):
    _exec("UPDATE users SET phone=%s WHERE telegram_id=%s",(phone,tg_id))

def get_user_by_tg(tg_id: int):
    with _conn() as cn, cn.cursor(cursor_factory=DictCursor) as cur:
        cur.execute("""SELECT user_id AS id, telegram_id, name, phone, balance FROM users WHERE telegram_id=%s""",(tg_id,))
        return cur.fetchone()
...
def get_product(pid:int):
    with _conn() as cn, cn.cursor(cursor_factory=DictCursor) as cur:
        cur.execute("""SELECT product_id AS id, name, price, description, photo_file_id FROM products WHERE product_id=%s""",(pid,))
        return cur.fetchone()
...
def mark_order_paid(order_id:int):
    _exec("UPDATE orders SET status='paid' WHERE order_id=%s",(order_id,))

# ---------- wallet ----------
def add_wallet_tx(user_id:int, kind:str, amount:float, meta:dict):
    with _conn() as cn, cn.cursor() as cur:
        cur.execute(
            "INSERT INTO wallet_transactions(user_id,kind,amount,meta) VALUES(%s,%s,%s,%s)",
...
# ---------- game ----------
def can_take_daily_bonus(user_id:int)->bool:
    with _conn() as cn, cn.cursor() as cur:
        cur.execute("SELECT 1 FROM daily_bonus WHERE user_id=%s AND day=CURRENT_DATE",(user_id,))
        return cur.fetchone() is None

def take_daily_bonus(user_id:int, amount:int)->bool:
    with _conn() as cn, cn.cursor() as cur:
        try:
            cur.execute("INSERT INTO daily_bonus(user_id,day,amount) VALUES(%s,CURRENT_DATE,%s)",(user_id,amount))
            cur.execute("INSERT INTO wallet_transactions(user_id,kind,amount,meta) VALUES(%s,'game',%s,'{\"game\":\"daily\"}'::jsonb)",(user_id,amount))
            return True
        except Exception:
            return False
